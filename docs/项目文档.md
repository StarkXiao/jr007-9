...
| 信用事件 | credit event | 信用分的一次变动记录（类型、分值、变动后分数、关联对象） |
| 信用等级 | credit tier | 由信用分推导的权限档位：优质 / 正常 / 受限 / 限制发布 |
...
| F11 | 信用分体系 | 信用事件流水、违规/申诉/通过率计分、按等级动态调整发布权限 |
...
命中以下任一条，标记为 `auto_rejected` 并给出具体原因（用户可点"仍然提交人工复核"覆盖，但会降低该用户信用分；信用等级为"限制"的用户没有此入口，只能按预检意见修改后再提交）：
...
- **分级审核策略**：
  - 信用分 < 60 的用户 → 评论先进入 `pending`，审核后可见；
  - 信用分 60–79 且历史通过评论 ≥ 3 条 → 先发后审，实时可见；
  - 信用分 ≥ 80 的优质贡献者 → 直接信任，先发后审；
...
- **定时任务**（BullMQ 队列 + 调度）：
  ...
  - `credit_sweep`：每日按历史通过率对信用分做幂等修正（见 5.11）；
...
信用分不直接惩罚人，而是决定**发布权限范围**：分数越低，能发的东西越少、越需要人工把关。全部分数变动都可追溯、可解释。

#### 5.11.1 信用事件（计分规则）

每一次分数变动落一条 `credit_events` 记录（类型、分值、变动后分数、关联对象），用户与管理端均可查看。

| 事件 | 分值 | 说明 |
|---|---|---|
| `spot_approved` | +2 | 条目通过审核 |
| `spot_rejected` | −8 | 条目被驳回（违规记录） |
| `spot_auto_rejected_override` | −5 | 自动预检未通过仍坚持转人工复核 |
| `comment_approved` | +1 | 先审后发的评论通过审核（低分用户的回升通道） |
| `comment_hidden` | −10 | 评论被隐藏（违规记录） |
| `report_confirmed` | −15 | 举报成立（违规记录，含条目下架、评论隐藏、图片下架、对人处罚） |
| `appeal_approved` | +8 | 申诉改判通过，**全额回抵**原驳回的 −8——误判不让用户长期背着 |
| `appeal_rejected` | −2 | 申诉维持原判 |
| `pass_rate_adjustment` | ±2 | 历史通过率修正（每日 `credit_sweep` 巡检） |

**历史通过率修正**：已决提交 ≥ 5 条时启用。通过率 ≥ 85% 加 2 分，≤ 40% 扣 2 分，中间档不修正。修正是**幂等**的：系统记录"目标修正分"与"已应用修正分"的差值，通过率回到中间档时之前的加减分自动退回。被申诉推翻的驳回不计入分母。

#### 5.11.2 信用等级与发布权限

| 等级 | 分数线 | 每日提交 | 每条图片 | 上传图片 | 评论 | 预检未通过转人工 |
|---|---|---|---|---|---|---|
| 优质 `trusted` | ≥ 80 | 20 | 6 | 允许 | 直接发布 | 允许 |
| 正常 `standard` | 60–79 | 20 | 6 | 允许 | 积累 3 条过审后直接发布 | 允许 |
| 受限 `limited` | 40–59 | 5 | 3 | 允许 | 一律先审 | 允许 |
| 限制 `restricted` | < 40 | 0（暂停新发布） | 0 | 禁止 | 一律先审 | 禁止 |

- 权限在服务端四个入口强制：创建条目（每日上限）、编辑条目（图片数）、上传图片、提交审核（转人工复核）、发表评论（分级审核）。
- **回升通道**：限制档用户没有被判死刑——评论（先审后发）仍然开放，评论过审 +1，通过率回升后每日巡检也会把分数拉回。这符合"每个状态都有出口"。
- 评论免预审分数线由 `PREMODERATE_CREDIT_THRESHOLD`（默认 60）配置，每日提交满额上限由 `DAILY_SPOT_LIMIT`（默认 20）配置。

#### 5.11.3 入口与反馈

- `GET /me/credit`：我的分数、等级、当前权限、历史通过率与最近 20 条信用事件，前端"我的空间 → 信用与权限"页展示。
- `GET /admin/users/:uuid/credit-events`：管理端查看任意用户的信用事件流水。
- 分数变动伴随业务通知（审核结果、申诉结果等），不单独发"扣分通知"。
...
CREATE TABLE credit_events (
  id          BIGSERIAL PRIMARY KEY,
  user_id     BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  kind        VARCHAR(32) NOT NULL,        -- 事件类型，见 5.11.1
  delta       SMALLINT NOT NULL,           -- 本次变动分值
  score_after SMALLINT NOT NULL,           -- 变动后的信用分
  target_type VARCHAR(24),                 -- 关联对象：spot / comment / media / user
  target_id   BIGINT,
  note        TEXT,                        -- 补充说明（如通过率修正时的通过率）
  created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
| GET | `/me/credit` | user | 我的信用分、等级、发布权限、通过率与信用事件 |
...
| GET | `/admin/users/:uuid/credit-events` | admin | 某用户的信用事件流水 |
